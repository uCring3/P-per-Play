...
function hamachi:load()
	...
	self.link.web = {
		img = love.graphics.newImage("menu/tutorials/web.png"),
		x = menu.hamachi.x,
	...
	self.link.youtube = {
		img = love.graphics.newImage("menu/tutorials/youtube.png"),
	...
	self.app.icona = {
		img = love.graphics.newImage("menu/tutorials/hamachi_icona.png"),
		x = 800,
		y = 100
	}
	...
	for i,img in ipairs(self.app.finestra.img) do
		self.app.finestra.img[i] = love.graphics.newImage("menu/tutorials/hamachi_finestra"..i..".png")
...
[[UNA VOLTA INSTALLATO HAMACHI BASTERA'
 CLICCARE SULL ICONA      PER ACCENDERLO]], self.bar.x*1.1, self.bar.y*1.3,	0.4, 0.4,		0.07, 0, 0)
	...
	elseif self.step == 12 then
		LOAD_TEXT_ANIM(
[[LA FINESTRA CONTIENE L'IP DEL VOSTRO COMPUTER
         (NON CONDIVIDERE CON GLI SCONOSCIUTI)]], self.bar.x*1.1, self.bar.y*1.4,	0.35, 0.35,		0.07, 0, 0)
...
[[UNA VOLTA FATTO QUESTO IL CREATORE DELLA STANZA
 DOVRA' DIRE IL SUO IP DEL COMPUTER
...
[[APERTO IL GIOCO BASTERA' CHE IL CREATORE DELLA STANZA
 ...
 (CONFERMA CON INVIO)]], self.bar.x*1.1, self.bar.y*1.3,	0.25, 0.25,		0.069, 0, 0)
	...
	elseif self.step == 22 then
		LOAD_TEXT_ANIM(
[[IL CODICE IP E' SIMILE A QUALCOSA COME 
...
function hamachi:draw()							--✏️
	...
	if self.step == 21 and self.arrow.bool then
		love.graphics.draw(self.arrow.img, menu.hostIP.x, menu.hostIP.y+menu.hostIP.height*1.3, math.pi, 0.4, 0.4)
		love.graphics.draw(self.arrow.img, menu.clientIP.x, menu.clientIP.y+menu.clientIP.height*1.3, math.pi, 0.4, 0.4)
	elseif self.step == 25 and self.arrow.bool2 then
		love.graphics.draw(self.arrow.img, menu.player1.x+menu.player1.width, menu.player1.y-menu.player1.height*0.25, 0, 0.4, 0.4)
		love.graphics.draw(self.arrow.img, menu.player2.x+menu.player2.width, menu.player2.y-menu.player2.height*0.25, 0, 0.4, 0.4)
	end
