...
function general:danno(attaccato,attaccante)
	if attaccato.id == 38 then --Raiden
		RaidenRandom1 = love.math.random(1, 25)
		RaidenRandom2 = love.math.random(1, 15)
		if RaidenRandom == 1 then
			attaccante.HP = attaccante.HP - danno
		end
		if RaidenRandom2 ~= 1 then
	...
	for c,card in ipairs(inCampoCards) do
		if draw_area and (card.id == 22) then -- Cosmi
			card.abilita = false
		end
	end
...
function general:stordisci(stordito)
	...
	general:scambio_dati_carta()
	for c,card in ipairs(inCampoCards) do
		if draw_cono and (card.id == 7) then --Shadow grimm
			card.abilita = false
		end
	end
	general:scambio_dati_carta()
...
function general:avvelena(avvelenato)
	...
	for c,card in ipairs(inCampoCards) do
		if draw_smith and (card.id == 8) then --Nabboleone
			card.abilita = false
		end
	end
	general:scambio_dati_carta()
